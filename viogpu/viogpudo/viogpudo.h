...
public:
    HwDeviceInterface(_In_ VioGpuDod* pVioGpuDod) {m_pVioGpuDod = pVioGpuDod; m_Type = INVALID_DEVICE;}
    virtual ~HwDeviceInterface(void) {;}
...
{
public:
    VgaDevice(_In_ VioGpuDod* pVioGpuDod);
    ~VgaDevice(void);
...
{
public:
    GpuDevice(_In_ VioGpuDod* pVioGpuDod);
    ~GpuDevice(void);
    NTSTATUS SetCurrentMode(ULONG Mode, CURRENT_BDD_MODE* pCurrentBddMode);
    ULONG GetModeCount(void) {return m_ModeCount;}
    NTSTATUS SetPowerState(DEVICE_POWER_STATE DevicePowerState, DXGK_DISPLAY_INFORMATION* pDispInfo);
