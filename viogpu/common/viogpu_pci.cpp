    ...
    if (m_BaseVA == nullptr)
    {
        if (m_bPortSpace)
        {
            if(m_bIoMapped)
            {
                ...
                {
                   DbgPrint(TRACE_LEVEL_INFORMATION, ("[%s] mapped port BAR at %x\n", __FUNCTION__, m_BasePA.LowPart));                }
                else
                {
                   m_BaseVA = nullptr;
        ...
        else
        {
            ...
            if (Status == STATUS_SUCCESS)
            {
                DbgPrint(TRACE_LEVEL_INFORMATION, ("[%s] mapped memory BAR at %I64x\n", __FUNCTION__, m_BasePA.QuadPart));
            }
            else
            {
                m_BaseVA = nullptr;
...
bool CPciResources::Init(PDXGKRNL_INTERFACE pDxgkInterface, PCM_RESOURCE_LIST pResList)
{
    ...
    Status = m_pDxgkInterface->DxgkCbReadDeviceSpace(m_pDxgkInterface->DeviceHandle,
                                                   ...
                                                   sizeof(pci_config),
                                                   &BytesRead);
    DbgPrint(TRACE_LEVEL_INFORMATION, ("<--> %s BytesRead = %d\n", __FUNCTION__, BytesRead));
    ...
    DbgPrint(TRACE_LEVEL_INFORMATION, ("---> %s ListCount = %d\n", __FUNCTION__, pResList->Count));
        ...
        for (ULONG j = 0; j < pFullResDescriptor->PartialResourceList.Count; ++j)
        {
           ...
           switch (pResDescriptor->Type)
           {
              case CmResourceTypePort:
              {
                 ...
                 bool bIoMapped = (pResDescriptor->Flags & CM_RESOURCE_PORT_IO) ? false : true;
                 bar = virtio_get_bar_index(&pci_config, Start);
                 DbgPrint(TRACE_LEVEL_INFORMATION, ("Found IO ports at %08lX(%d) bar %d\n", Start.LowPart, len, bar));
                 if (bar < 0)
                 {
                    break;
                 }
              ...
              case CmResourceTypeInterrupt:
              {
                 ...
                 if (IsMSIEnabled())
                 {
                     DbgPrint(TRACE_LEVEL_INFORMATION, ("Found MSI Interrupt vector %d, level %d, affinity 0x%X, flags %X\n",
                ...
                {
                    DbgPrint(TRACE_LEVEL_INFORMATION, ("Found Interrupt vector %d, level %d, affinity 0x%X, flags %X\n",
              ...
              case CmResourceTypeMemory:
              {
                 ...
                 ULONG len = pResDescriptor->u.Port.Length;
                 bar = virtio_get_bar_index(&pci_config, Start);
                 DbgPrint(TRACE_LEVEL_INFORMATION, ("Found IO memory at %08I64X(%d) bar %d\n", Start.QuadPart, len, bar));
                 if (bar < 0)
                 {
                    break;
                 }
